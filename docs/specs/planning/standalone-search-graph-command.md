...
Yêu cầu mới là tách phần Search page ra một frontend source riêng, vẫn dùng Vue, Prettier, ESLint, Tailwind/DaisyUI-style CSS, icon system và network graph hiện tại, nhưng có khả năng chạy standalone/on the fly. Đồng thời thêm command `graph` để sinh một file HTML vào thư mục hiện tại và mở bằng web browser mặc định.
...
Hướng thiết kế nên giữ một backend search contract duy nhất thay vì nhân đôi logic search sang frontend. Lệnh `graph` chỉ nên tạo shell HTML/JS standalone và, khi cần dữ liệu động từ project, dùng API/runtime Go đã có hoặc snapshot JSON được nhúng vào HTML theo chế độ được chọn.
...
- Có lệnh `go run . graph` sinh file HTML vào `pwd` và mở bằng browser mặc định.
- File HTML sinh ra có tên ổn định, ví dụ `search-graph.html` hoặc `ns-workspace-graph.html`, và có thể được ghi đè an toàn.
...
Lệnh `graph` nên mặc định dùng project root là thư mục hiện tại. Các flag đề xuất:
...
1. Runtime local server nhẹ: `graph` sinh HTML vào `pwd`, đồng thời chạy một local server API tạm giống preview để HTML gọi `/api/search`, `/api/docs`, `/api/files`. Hướng này giữ dữ liệu luôn động và ít thay đổi frontend search logic nhất, nhưng command trở thành long-running giống `preview`.
2. Snapshot HTML: `graph` gọi backend để build một snapshot dữ liệu ban đầu và nhúng JSON vào HTML. Hướng này tạo file tự đủ hơn, nhưng search code theo query mới sẽ khó vì hiện backend tính search theo query và có embedding/graphify/Git filtering ở Go runtime.

Đề xuất chọn hướng 1 cho triển khai đầu: lệnh `graph` tạo HTML launcher vào `pwd`, start server bằng API search hiện có, mở browser tới URL server hoặc file launcher. Nếu bắt buộc browser mở trực tiếp file HTML, HTML có thể chứa endpoint server được inject và redirect/connect tới server đang chạy.

## Cấu Trúc Giải Pháp

```mermaid
flowchart LR
  CLI["go run . graph"] --> GraphRun["preview.RunGraph()"]
  GraphRun --> HTML["ghi standalone HTML vào pwd"]
  GraphRun --> Server["local API server"]
...
- Nếu `graph` chỉ mở file HTML nhưng không giữ server sống, search động theo query sẽ không hoạt động vì logic search nằm ở Go backend. Cần quyết định rõ: file HTML là launcher tới local server hay là snapshot thật sự.
...
- `go run . graph --project <fixture> --out <tmp>/search-graph.html --no-open` tạo đúng file HTML.
