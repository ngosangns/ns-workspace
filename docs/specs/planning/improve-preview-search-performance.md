...
Đo nhanh trên `ns-workspace` cho thấy mỗi query khoảng 220-260ms. Đo trên project lớn hơn `viclass` cho thấy mỗi query khoảng 4.3-4.8s. Corpus của `viclass` có khoảng 4.9k tracked files, 3.6k code-like files, 584 Markdown/HTML tracked files, và `graphify-out/graph.json` khoảng 41MB với 27k nodes/53k links.
...
- Project lớn như `viclass`: giảm từ 4.3-4.8s/query xuống dưới 800ms cho query nóng sau lần warm-up.
...
- Chạy validation và benchmark trước/sau trên `ns-workspace` và `viclass`.
...
- Đo trước/sau bằng local server:
  ...
  - `go run . preview --no-reload --project /Users/ngosangns/Github/viclass`
  - `curl /api/search?q=search`, `preview`, `graph`, `auth`, `class`
- Kỳ vọng:
  - `ns-workspace`: query nóng không tệ hơn hiện tại.
  - `viclass`: query nóng giảm từ khoảng 4.3-4.8s xuống dưới 800ms.
- Chạy:
