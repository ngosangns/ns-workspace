...
- Package `internal/graphquery`: registry LSP install/status, cache path, installer wrappers và CLI `lsp`.
- `internal/preview/preview_lsp.go`: resolver tái sử dụng cache path do `internal/graphquery` khai báo.
...
- Không tải binary archive nếu không có checksum/version pin rõ. Installer archive hiện chỉ dùng cho Kotlin LSP với SHA-256 pinned theo OS/arch.

## Logic Nghiệp Vụ

### Registry ngôn ngữ

`internal/graphquery` có model nhỏ dùng chung cho resolver, status và installer:
...
- `typescript`: `.ts`, `.tsx`, `.js`, `.jsx`, `.cjs`, `.mjs`, binary `typescript-language-server`, args `--stdio`, check `--version`, install qua `npm install --prefix <cache>/typescript typescript-language-server typescript`.
- `html`: `.html`, `.htm`, binary `vscode-html-language-server`, args `--stdio`, install qua `npm install --prefix <cache>/html vscode-langservers-extracted`.
- `css`: `.css`, `.scss`, `.sass`, binary `vscode-css-language-server`, args `--stdio`, install qua `npm install --prefix <cache>/css vscode-langservers-extracted`.
- `kotlin`: `.kt`, `.kts`, binary `kotlin-lsp`, args `--stdio`, install qua JetBrains standalone archive pinned theo version/checksum.

Các `lspLanguage` runtime trong preview được sinh từ `graphquery.LanguageSpecs()` để tránh lệch giữa runtime và installer.
...
- Check prerequisite:
  - Go: `go version`.
  - TypeScript/HTML/CSS: `node --version`, `npm --version`.
  - Kotlin: archive source phải hỗ trợ OS/arch hiện tại và có checksum pinned.
- Cài vào cache:
  ...
  - HTML/CSS: chạy `npm install --prefix <cache>/<id> vscode-langservers-extracted`.
  - Kotlin: tải archive, verify SHA-256, extract vào cache versioned và ghi wrapper `kotlin-lsp`.
...
- Cài những server thiếu có installer trong registry.
- Với prerequisite/install failure, batch auto-install trả result/warning và vẫn để graph query fail-open; exit non-zero chỉ khi user chỉ định language cụ thể và install thất bại.
...
flowchart LR
  ...
  LSPCLI --> Ensure
  Ensure --> Registry["internal/graphquery LSP registry"]
...
### 1. Thêm registry LSP cho graph query

Tạo package `internal/graphquery` để chứa:
...
Không kéo abstraction quá rộng. Registry hiện phục vụ nhóm language đã yêu cầu và vẫn đủ rõ để thêm language khác sau. `internal/preview/preview_lsp_setup.go` chỉ còn adapter mỏng cung cấp detector source file và alias type cho runtime preview/search.

### 2. Thêm service cài đặt và status

Tạo service trong `internal/graphquery` với các hàm tương đương:
...
`previewLSPManager.commandCandidates()` tái sử dụng cache dirs từ `internal/graphquery`:

- TypeScript: `<cache>/typescript/node_modules/.bin/typescript-language-server`.
- Go: `<cache>/go/bin/gopls`.
- HTML/CSS: `<cache>/<id>/node_modules/.bin/vscode-*-language-server`.
- Kotlin: `<cache>/kotlin/bin/kotlin-lsp`.

Các cache dirs đi qua helper `graphquery.CacheCommandDirs()` để preview không hardcode installer layout.
...
- Luồng setup dùng interface `lspImplementation`; mỗi server LSP tự khai báo language specs, install spec, cache dirs, dry-run command và routine cài đặt.
...
- Kotlin tải JetBrains Kotlin LSP standalone archive theo OS/arch, verify SHA-256 pinned, extract vào cache versioned và ghi wrapper `<cache>/kotlin/bin/kotlin-lsp`.
...
1. Registry LSP trong `internal/graphquery` hỗ trợ HTML, CSS/SCSS/Sass, JavaScript/TypeScript, Go/Golang và Kotlin qua interface `lspImplementation`, với implementation riêng cho từng server LSP.
...
4. Installer hiện có:
   ...
   - `kotlin-lsp` qua JetBrains standalone archive pinned theo version/checksum; `lsp install kotlin` trả status `installed` khi archive tải, verify, extract và wrapper check thành công.
...
- TypeScript, HTML và CSS language servers cần Node/npm; Go language server cần Go toolchain. Missing prerequisite phải rõ, không che bằng warning generic. Kotlin installer cần network ổn định và artifact/checksum đúng với OS/arch được hỗ trợ.
