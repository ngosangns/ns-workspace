...
- Không auto-install LSP server trong HTTP `/api/search`, `preview` hoặc `search` UI. Nếu binary thiếu, các bề mặt này fail-open và trả warning có command cài đặt; CLI setup nằm ở `lsp install`, còn `graph --query` tự ensure LSP theo mặc định.
...
- Missing LSP server không làm `/api/search` lỗi; response có warning kèm command `lsp install <language>`. CLI `graph --query` tự ensure LSP trước khi query trừ khi dùng `--no-ensure-lsp`.
