...
- Không auto-install `gopls`, `typescript-language-server` hoặc language server khác trong preview/search HTTP request. CLI setup được xử lý bởi `lsp install`, còn `graph --query` tự ensure LSP theo mặc định.
...
/Users/ngosangns/.agents/skills/lsp-code-graph/SKILL.md
```
...
- `/Users/ngosangns/.agents/skills/...` tương ứng
...
- `rg -n "lsp-code-graph|Preview/Search Code Graph|graph --project" presets/skills /Users/ngosangns/.agents/skills` để xác nhận các skill route đúng.
