...
- **Links**: [Chỉ mục](../_index.md), [Preview web](../features/preview-web.md), [Kiến trúc tổng quan](../architecture/overview.md), [HTML docs](../specs/planning/generate-docs-html-tailwind-custom-tags.md), [Mermaid/C4 rendering](../specs/planning/support-mermaid-and-c4-model-rendering.md), [Directed Code Graph Search](../specs/planning/improve-code-graph-call-flow-readability.md), [Search backend ranking](../specs/planning/improve-preview-search-backend-ranking.md), [Renderer graph preview](../specs/planning/use-specialized-graph-renderer.md), [Quy ước frontend preview](../development/conventions/preview-frontend.md)
...
- Docs Graph và Code Graph search trực tiếp trên docs graph hoặc graphify graph bằng query hiện tại, độc lập với semantic results, rồi trả neighbors quanh node match để UI hiển thị context. Code Graph bỏ graphify file-only nodes, chỉ giữ callable code nodes có source file tracked bởi Git, normalize graphify `source_file` trước khi so với `git ls-files`, không fallback sang untracked graphify nodes khi không đọc được Git tracked files, không đưa doc, untracked nodes hoặc class/container nodes trở lại qua neighbor rendering, và sort direct symbol matches theo score/evidence/title/path trước khi mở rộng từng anchor qua directed `calls` để trả caller hoặc callee liên quan trong cùng panel. Quan hệ `method` và symbol-level `contains` chỉ dùng để tìm owner label cho callable node, nên method hiển thị dạng `ClassName.methodName()` mà class không thành node riêng. Direct incoming `calls` được đánh dấu là caller, direct outgoing `calls` được đánh dấu là callee và được ưu tiên trước expansion sâu hơn; UI vẽ border màu đen trong light theme và trắng trong dark theme cho caller node ở đầu nguồn của graph đang render, tức node có outgoing `calls` nhưng không có incoming `calls`.
