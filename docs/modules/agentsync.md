...
Stable adapters hiện gồm Claude Code, Claude Desktop (app GUI), OpenCode, Grok Build, Kimi Code CLI, Kiro/Kiro CLI, Qwen Code, Antigravity CLI (`agy`), Codex CLI, Cline, ZCode, Hermes Agent và Pi coding agent (`pi`). Stable adapters ghi hoặc link/copy trực tiếp tới native user-level locations. Adapter phổ thông đi qua `AdapterSpec`; adapter có logic riêng dùng plugin nhỏ.
...
- OpenCode / ZCode / Codex / Kimi / Pi: skills chỉ qua `~/.agents/skills` (không mirror native skills dir). OpenCode vẫn link AGENTS.md + subagents + merge MCP; ZCode/Pi link AGENTS.md; stale symlink cũ dưới `~/.config/opencode/skill`, `~/.grok/skills`, `~/.zcode/skills` được cleanup nếu trỏ vào shared home.
...
- Pi coding agent (`pi`, alias `pi-agent` / `pi-coding-agent`): link shared `AGENTS.md` → `$PI_CODING_AGENT_DIR/AGENTS.md` (default `~/.pi/agent`). Skills load native từ `~/.agents/skills` và optional `~/.pi/agent/skills` — không mirror. Không ghi MCP (pi không first-party MCP) và không overwrite `settings.json` / `models.json` / extensions. Tôn trọng `PI_CODING_AGENT_DIR`.
...
- `presets/settings/antigravity.json`: `{ "toolPermission": "always-proceed", "artifactReviewPolicy": "always-proceed" }` — full auto-approve tools + artifact writes (Antigravity CLI settings tại `~/.gemini/antigravity-cli/settings.json`).
...
- `presets/adapters/antigravity.json`: raw profile, copy `presets/settings/antigravity.json` sang `.gemini/antigravity-cli/settings.json`. MCP không nest trong settings — ghi riêng qua `MCPPath` → `~/.gemini/config/mcp_config.json`.
...
| Antigravity | `toolPermission` + `artifactReviewPolicy` | `"always-proceed"` | Full auto-approve tools and artifact writes ([CLI settings](https://antigravity.google/docs/cli/settings)). |
...
- **antigravity**: remote HTTP/SSE dùng `serverUrl` (không `url`/`httpUrl`), drop `type`; stdio giữ `command`+`args`. Ghi vào `~/.gemini/config/mcp_config.json`. Theo [Antigravity MCP docs](https://antigravity.google/docs/mcp) và [migration](https://antigravity.google/docs/cli/gcli-migration).
