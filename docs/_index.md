...
Planning/spec hiện có: [Tách Search Page Thành Frontend Standalone Và Thêm Lệnh Graph](./specs/planning/standalone-search-graph-command.md), [Thay Code Graph Graphify Bằng LSP](./specs/planning/lsp-code-graph-search.md) và [Đóng Gói LSP Search Graph Thành Command Và Skill](./specs/planning/package-lsp-search-graph-command-skill.md). Hành vi đã shipped được mô tả trực tiếp trong [Preview web](./features/preview-web.md), [Module preview](./modules/preview.md) và [Kiến trúc tổng quan](./architecture/overview.md).
