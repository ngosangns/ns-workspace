...
- Preset agent instruction trong `presets/agents/AGENTS.md` nhận trigger skill dạng `//<tag>` cho pipeline research, search docs, plan, execution, fix và update-docs. Trigger riêng `/s` gọi skill `spawn-opencode` để spawn OpenCode process như sub-agent.
