# Graph Report - ~/path/to/ns-workspace  (2026-05-08)
...
- `specsRoot()` --calls--> `expandPath()`  [INFERRED]
  ~/path/to/ns-workspace/spec_project.go → ~/path/to/ns-workspace/main.go
- `runPreview()` --calls--> `expandPath()`  [INFERRED]
  ~/path/to/ns-workspace/preview.go → ~/path/to/ns-workspace/main.go
- `run()` --calls--> `runPreview()`  [INFERRED]
  ~/path/to/ns-workspace/main.go → ~/path/to/ns-workspace/preview.go
- `TestPreviewHTTPHandlers()` --calls--> `newPreviewServer()`  [INFERRED]
  ~/path/to/ns-workspace/preview_test.go → ~/path/to/ns-workspace/preview.go
...
- **Are the 6 inferred relationships involving `scanSpecProject()` (e.g. with `.load()` and `TestScanSpecProjectParsesTableStyleIndex()`) actually correct?**
