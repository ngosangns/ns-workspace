...
var defaultScripts = []scriptSpec{
	...
	{
		Name:        "ns:preview",
		Description: "Chạy web dashboard local cho docs/ của project hiện tại",
	...
	{
		Name:        "lint:docs",
		Description: "Lint markdown + html trong docs và presets",
		Commands:    []string{"npm", "run", "lint:docs"},
	},
	{
		Name:        "lint:docs:fix",
		Description: "Auto-fix markdown + html lint issues",
		Commands:    []string{"npm", "run", "lint:docs:fix"},
	},
	{
		Name:        "lint:preview",
		Description: "Lint TypeScript + Vue trong preview UI",
	...
	// Nhóm format:* — npm scripts format code.
	{
		Name:        "format:docs",
		Description: "Format markdown + html bằng Prettier",
		Commands:    []string{"npm", "run", "format:docs"},
	},
	{
		Name:        "format:docs:check",
		Description: "Check format markdown + html (không ghi file)",
		Commands:    []string{"npm", "run", "format:docs:check"},
	},
	{
		Name:        "format:preview",
		Description: "Format TypeScript + Vue + config files bằng Prettier",
		Commands:    []string{"npm", "run", "format:preview"},
	},
	{
		Name:        "format:preview:check",
		Description: "Check format TypeScript + Vue (không ghi file)",
		Commands:    []string{"npm", "run", "format:preview:check"},
	},

	// Nhóm build:* — npm scripts build artifact.
	{
		Name:        "build:preview",
		Description: "Build UI preview với Vite",
		Commands:    []string{"npm", "run", "build:preview"},
	},
...
func Run(args []string) error {
	...
	fs.SetOutput(io.Discard) // Mặc định flag tự in help ra stderr khi có lỗi; tắt để tránh noise.
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), "Usage: setup [--target DIR] [--dry-run] [--force]")
		fs.PrintDefaults()
	}
	...
	return writeTaskfile(target, defaultScripts, dryRun, force, os.Stdout)
}
...
func writeTaskfile(target string, scripts []scriptSpec, dryRun, force bool, stdout io.Writer) error {
	...
	if err := os.WriteFile(taskfilePath, []byte(body), 0o644); err != nil {
...
// DefaultScripts trả về danh sách scripts hard-coded. Export để test và để
// command khác (nếu có) có thể tham chiếu.
func DefaultScripts() []scriptSpec {
	out := make([]scriptSpec, len(defaultScripts))
	copy(out, defaultScripts)
	return out
}
