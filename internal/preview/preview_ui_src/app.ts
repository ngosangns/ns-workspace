...
function validSpecFolderPath(path) {
  ...
  if (normalized === "docs") return state.specs.length ? normalized : "";
...
async function selectSpecFolder(path, showSpecTab, options: UpdateURLOptions = {}) {
...
function renderSpecFolderContent(root: HTMLElement, folderPath: string) {
  ...
  `;
  root.querySelectorAll<HTMLElement>("[data-folder-path]").forEach((button) => {
    button.addEventListener("click", () =>
      selectSpecFolder(button.dataset.folderPath || "", true).catch((error) => renderPreviewError(error)),
    );
  });
  root.querySelectorAll<HTMLElement>("[data-doc-id]").forEach((button) => {
    button.addEventListener("click", () => selectSpec(button.dataset.docId || "", true).catch((error) => renderPreviewError(error)));
  });
  refreshIcons();
}

function specFolderListing(folderPath: string) {
...
function renderDocListingCard(spec: SpecDocument) {
  return `
