...
async function renderMarkdownPreview(root, raw) {
    ...
    const viewerHost = root.querySelector("[data-markdown-viewer]");
    if (!viewerHost)
        return;
    ...
    const markdownRoot = viewerHost.querySelector(".toastui-editor-contents") || viewerHost;
...
function normalizeHTMLDocTags(root) {
...
function replaceDocElement(node, tagName, className) {
    const element = document.createElement(tagName);
    element.className = className;
    moveNodeChildren(node, element);
    node.replaceWith(element);
}
function replaceDocContainer(node, tagName, className, titleTagName) {
    const element = document.createElement(tagName);
    element.className = className;
...
function replaceDocMetric(node) {
...
function createDocDiagramSource(source, language, className) {
    const code = document.createElement("code");
    code.className = `language-${language}`;
    code.dataset.sourceLanguage = language;
...
function moveNodeChildren(from, to) {
    while (from.firstChild) {
        to.append(from.firstChild);
    }
}
function normalizeDocCodeLanguage(language) {
    return sanitizeClassToken(language || "plaintext") || "plaintext";
}
function normalizeDocDiagramLanguage(language) {
    const normalized = sanitizeClassToken(language).replace(/-/g, "");
    if (normalized === "c4model" || normalized === "likec4")
        return "likec4";
    if (normalized === "c4")
        return "c4";
...
function renderToastMarkdownLoading(label) {
    return `
...
async function loadToastMarkdownViewer() {
    if (state.markdownViewerConstructor)
        return state.markdownViewerConstructor;
    const moduleURL = "https://esm.sh/@toast-ui/editor@3.2.2/dist/toastui-editor-viewer?bundle&target=es2022";
    const viewerModule = (await import(moduleURL));
...
function markdownFenceLanguage(node) {
