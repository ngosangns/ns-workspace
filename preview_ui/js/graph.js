export function createDocsGraph({ state, els, escapeHTML, refreshIcons, selectSpec }) {
  return {
    render,
    stop,
  };
  ...
  function render() {
    ...
    const simulation = d3.forceSimulation(graph.nodes)
      .force("link", d3.forceLink(graph.links).id((n) => n.id).distance((edge) => (edge.type === "depends" ? 120 : 96)))
    ...
    const node = viewport
      ...
      .on("click", (_, n) => selectGraphNode(n.id));
  ...
  function renderDetails(graph) {
    ...
    `;
    els.graphDetails.querySelector("[data-open-spec]")?.addEventListener("click", (event) => {
      selectSpec(event.currentTarget.dataset.openSpec, true);
    });
    els.graphDetails.querySelectorAll("[data-select-node]").forEach((button) => {
      button.addEventListener("click", () => selectGraphNode(button.dataset.selectNode));
    });
    refreshIcons();
  }
