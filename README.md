...
`preview` starts a localhost-only web server that reads a project-style `specs/` folder and renders a DaisyUI dashboard with the spec list, Markdown preview, sync state, dependency graph, relationship map, and forbidden dependency rules. Markdown code fences for `mermaid` render as diagrams in the browser.
