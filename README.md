...
The embedded custom presets are imported from this user's `~/.agents` folder plus `~/path/to/project/.agents`, including project agent definitions, hooks, private skills, and the project `.mcp.json` servers. Skills that already exist in the public Skills registry are installed from upstream instead of being embedded.
...
`preview` starts a localhost-only web server that reads a project-style `specs/` folder and renders a Notion-like dashboard with the spec list, Markdown preview, sync state, dependency graph, relationship map, and LikeC4 models. Markdown code fences for `mermaid`, `likec4`, `likec4-dsl`, and `c4` render as diagrams; the Models tab scans `.c4`/`.likec4` workspaces or generates a LikeC4 model from the spec graph. LikeC4 rendering uses the official `likec4 gen mermaid` CLI through `npx`, then displays through Mermaid in the browser.
