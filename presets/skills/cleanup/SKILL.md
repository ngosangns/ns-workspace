---
name: cleanup
description: Audit worktree/branch/commit để đánh giá dead code, dead flows, dead docs, legacy artifacts. Output là plan trong `docs/developer/specs/planning/`. Trigger: dọn dẹp, audit branch, tìm dead code.
---
...
- File plan trong `docs/developer/specs/planning/cleanup-<scope>.md`.
...
2. Chạy `read-search-docs`: đọc `AGENTS.md`, `docs/README.md`, `docs/_index.md`, `docs/_sync.md`, specs/features/modules liên quan (cả `business/` và `developer/`).
...
6. Dùng `plan` tạo file `cleanup-<scope>.md` trong `docs/developer/specs/planning/`. Plan bằng tiếng Việt, nêu bối cảnh, phạm vi, candidate, thứ tự, rủi ro, validation.
